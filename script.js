...
function addcart(productImg, productName, productPrice) {
  ...
  var cartItem = document.querySelectorAll("tbody tr");
  for (var i = 0; i < cartItem.length; i++) {
    if (cartItem[i].querySelector(".title").innerText === productName) {
  ...
  var trcontent = `
    <tr>
      <td style="display: flex; align-items: center;">
        <img src="${productImg}" style="width: 70px;">
      ...
      <td style="cursor: pointer;">
        <span class="cart-delete" style="padding: 10px; boder: none; border-radius: 15px; background: #eb2632; color: white;">Remove</span>
      </td>
    </tr>
  `;
  addtr.innerHTML = trcontent;
...
function updateTotal() {
  ...
  // Update total in payment.html
  var paymentTotal = document.querySelector(".price-total span");
  paymentTotal.innerHTML = total.toLocaleString("de-DE");
}
